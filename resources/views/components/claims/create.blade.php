...
<div class="container py-5" style="max-width: 1100px;">
    <div class="bg-white p-4 shadow rounded">
        ...
        <form method="GET" action="{{ route('claims.create') }}" class="needs-validation" novalidate>
            <div class="row g-3">
                <div class="col-md-6">
                    <label for="id_no" class="form-label fw-semibold">id_no Number</label>
                    <div class="input-group">
                        <span class="input-group-text"><i class="bi bi-person-vcard"></i></span>
                        <input type="text" class="form-control" id="id_no" name="id_no" 
                               value="{{ old('id_no') }}" placeholder="Enter id_no number">
                    </div>
                </div>
                <div class="col-md-6">
                    <label for="passport_no" class="form-label fw-semibold">passport_no Number</label>
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="passport_no" name="passport_no" 
                               value="{{ old('passport_no') }}" placeholder="Enter passport_no number">
                    </div>
                </div>
        ...
        <form method="POST" action="{{ route('claims.store', $application->application_no) }}" enctype="multipart/form-data">
                ...
                <div class="row g-3 mt-2">
                    ...
                    <div class="col-md-6">
                        <x-overdue.textarea-group name="customer_address" label="Customer Address" 
                            value="{{ $application->customer_address }}" rows="2" readonly />
                    </div>
                </div>
            </section>
            ...
            <section class="mb-5">
                ...
                <x-overdue.textarea-group name="default_reasons" label="Reasons for Default" 
                    value="{{ $claim->default_reasons ?? old('default_reasons') }}" rows="4" required />
            ...
            <section class="mb-5">
                ...
                <x-overdue.textarea-group name="recovery_steps_taken" label="Steps Already Taken" 
                    value="{{ $claim->recovery_steps_taken ?? old('recovery_steps_taken') }}" rows="4" required />
                <x-overdue.textarea-group name="proposed_steps" label="Proposed Next Steps" 
                    value="{{ $claim->proposed_steps ?? old('proposed_steps') }}" rows="4" required />
                ...
                <div class="row g-3 mt-2">
                    ...
                    <div class="col-12">
                        <x-overdue.textarea-group name="bank_address" label="Bank Address" 
                            value="{{ $claim->bank_address ?? old('bank_address') }}" rows="2" required />
                    </div>
                </div>
            </section>
